...
int main()
{
    /*
        要求：
        完成先把二号信号进行屏蔽，并且屏蔽之后，也是一直保存到pending位图之中

        函数:
        内核之中的核心数据结构的操作函数
        1、int sigprocmask(int how, const sigset_t *set, sigset_t *oldset); 操作的就是block位图
        2、int sigpending(sigset_t *set);
     */

    /* 1、先尝试屏蔽指定信号,也就是操作修改block位图 */
